...
def ml_loop(side: str):
    ...
    def move_to(player, pred) : #move platform to predicted position to catch ball 
        if player == '1P':
            if scene_info["platform_1P"][0]+40  > (pred+2) and scene_info["platform_1P"][0] < pred: return 0 # NONE
            elif scene_info["platform_1P"][0]+40 <= (pred+2) : return 1 # goes right
    ...
    def ml_loop_for_1P(): 
        if scene_info["ball_speed"][1] > 0 : # 球正在向下 # ball goes down
            ...
            x = 155 // ball_NowspeedY # 幾個frame以後會需要碰到障礙物
            mod = 155 % ball_NowspeedY
            if mod != 0:
                x = x + 1
            ...
            if (bound > 0): # pred > 200 # fix landing position
                ...
                if (bound%2 == 1) :
                    ...
                    pred = pred + (abs(bound)*200)
            pred1 = pred + (scene_info["ball_speed"][0]*x)
            bound = pred1 // 200 # Determine if it is beyond the boundary
            if (bound > 0): # pred > 200 # fix landing position
                if (bound%2 == 0) : 
                    pred1 = pred1 - bound*200                    
                else :
                    pred1 = 200 - (pred1 - 200*bound)
            elif (bound < 0) : # pred < 0
                if (bound%2 == 1) :
                    pred1 = abs(pred1 - (bound+1) *200)
                else :
                    pred1 = pred1 + (abs(bound)*200)
            pred2 = pred + (scene_info["ball_speed"][0]*(-x))
            bound = pred2 // 200
            if (bound > 0): # pred > 200 # fix landing position
                if (bound%2 == 0) : 
                    pred2 = pred2 - bound*200                    
                else :
                    pred2 = 200 - (pred2 - 200*bound)
            elif (bound < 0) : # pred < 0
                if (bound%2 == 1) :
                    pred2 = abs(pred2 - (bound+1) *200)
                else :
                    pred2 = pred2 + (abs(bound)*200)
            print("pred1= ",pred1)
            print("pred2= ",pred2)
            if(scene_info["ball"][1]>=260):
                result = (pred1 + pred2) / 2
                return move_to(player = '1P',pred = result)
            else:
